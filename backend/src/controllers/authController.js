"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.login = exports.register = void 0;
var bcrypt_1 = require("bcrypt");
var Student_1 = require("../models/Student");
var Teacher_1 = require("../models/Teacher");
var jwt = require("jsonwebtoken");
var SALT_ROUNDS = 10;
var JWT_SECRET = "supersecretkey";
var register = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, name, email, password, role, userExists, passwordHash, newStudent, userExists, passwordHash, newTeacher, error_1;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _a = req.body, name = _a.name, email = _a.email, password = _a.password, role = _a.role;
                _b.label = 1;
            case 1:
                _b.trys.push([1, 10, , 11]);
                if (!(role === "student")) return [3 /*break*/, 5];
                return [4 /*yield*/, Student_1.default.findOne({ email: email })];
            case 2:
                userExists = _b.sent();
                if (userExists) {
                    return [2 /*return*/, res.status(409).json({ error: "Email already registered" })];
                }
                return [4 /*yield*/, bcrypt_1.default.hash(password, SALT_ROUNDS)];
            case 3:
                passwordHash = _b.sent();
                newStudent = new Student_1.default({
                    name: name,
                    email: email,
                    passwordHash: passwordHash,
                    role: role,
                    baseHP: 100,
                    currentHP: 100,
                });
                return [4 /*yield*/, newStudent.save()];
            case 4:
                _b.sent();
                return [2 /*return*/, res.status(201).json({ message: "Student registered successfully" })];
            case 5:
                if (!(role === "teacher")) return [3 /*break*/, 9];
                return [4 /*yield*/, Teacher_1.default.findOne({ email: email })];
            case 6:
                userExists = _b.sent();
                if (userExists) {
                    return [2 /*return*/, res.status(409).json({ error: "Email already registered" })];
                }
                return [4 /*yield*/, bcrypt_1.default.hash(password, SALT_ROUNDS)];
            case 7:
                passwordHash = _b.sent();
                newTeacher = new Teacher_1.default({
                    name: name,
                    email: email,
                    passwordHash: passwordHash,
                    role: role,
                });
                return [4 /*yield*/, newTeacher.save()];
            case 8:
                _b.sent();
                return [2 /*return*/, res.status(201).json({ message: "Teacher registered successfully" })];
            case 9: return [2 /*return*/, res.status(400).json({ error: "Invalid role specified" })];
            case 10:
                error_1 = _b.sent();
                console.error("Registration error:", error_1);
                res.status(500).json({ error: "Server error during registration" });
                return [3 /*break*/, 11];
            case 11: return [2 /*return*/];
        }
    });
}); };
exports.register = register;
var login = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, email, password, role, user, isValid, token, error_2;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _a = req.body, email = _a.email, password = _a.password, role = _a.role;
                _b.label = 1;
            case 1:
                _b.trys.push([1, 8, , 9]);
                user = void 0;
                if (!(role == 'student')) return [3 /*break*/, 3];
                return [4 /*yield*/, Student_1.default.findOne({ email: email })];
            case 2:
                user = _b.sent();
                return [3 /*break*/, 6];
            case 3:
                if (!(role === 'teacher')) return [3 /*break*/, 5];
                return [4 /*yield*/, Teacher_1.default.findOne({ email: email })];
            case 4:
                user = _b.sent();
                return [3 /*break*/, 6];
            case 5: return [2 /*return*/, res.status(400).json({ error: "Invalid role specified" })];
            case 6:
                if (!user) {
                    return [2 /*return*/, res.status(404).json({ error: "User not found" })];
                }
                return [4 /*yield*/, bcrypt_1.default.compare(password, user.passwordHash)];
            case 7:
                isValid = _b.sent();
                if (!isValid) {
                    return [2 /*return*/, res.status(401).json({ error: "Invalid credentials" })];
                }
                token = jwt.sigh({
                    id: user._id, name: user.name, email: user.email,
                    role: role
                }, JWT_SECRET, { expiresIn: '1h' });
                return [2 /*return*/, res.json({
                        message: "Login successful",
                        token: token,
                        role: role,
                    })];
            case 8:
                error_2 = _b.sent();
                console.error("Login error:", error_2);
                res.status(500).json({ error: "Server error during login" });
                return [3 /*break*/, 9];
            case 9: return [2 /*return*/];
        }
    });
}); };
exports.login = login;
